...
class TrackGenerator:
    def __init__(self, x: float, y: float, z: float, px: float, py: float, pz: float, charge: int):
    ...
    def evolve_track(self, time_steps=100):
        ...
        z_values = self.z + (self.pz / self._pT) * (self._radius * self._angular_frequency * times)
    ...
    def find_layer_intersections(self, verbose=False, max_time_steps=500):
        ...
        while time_steps <= max_time_steps:
            x_values, y_values, z_values, _, time_values = self.evolve_track(time_steps=time_steps)
            ...
            for layer_radius in self.pixel_layers:
                ...
                if crossing_idx is None:
                    ...
                    if verbose:
                        print(f"No crossing found for layer {layer_radius} mm with {time_steps} time steps")
                    continue
                    
                if verbose:
                    print(f"Found crossing at index {crossing_idx} for layer {layer_radius}")
                    
                hit_coords = self._interpolate_intersection(
                    x_values, y_values, z_values, r_track, 
                    crossing_idx, layer_radius
                )
            ...
            if verbose:
                print(f"Increasing time steps to {time_steps} to find missing layers: {missing_layers}")
    ...
    def _adjust_track_parameters(self, attempt, min_pt_needed, verbose):
        ...
        if verbose:
            print(f"Attempt {attempt + 1}: pT={self._pT:.2f}, center=({self._x_center:.2f}, {self._y_center:.2f})")
    ...
    def __repr__(self):
        ...
        """
        return (f"TrackGenerator(x={self.x:.2f}, y={self.y:.2f}, z={self.z:.2f}, "
