...
z = np.linspace(
    -sim.layer.length / 2, sim.layer.length / 2, sim.layer.LONGITUDINAL_STEPS
)
theta = np.linspace(0, 2 * np.pi, sim.layer.ANGULAR_STEPS)
Z, Theta = np.meshgrid(z, theta)

# Define colors for each sub-detector group
colors = {"Pixel": "mediumorchid", "SCT": "royalblue", "TRT": "royalblue"}
...
sim.ax.plot(
    z_vals, x_vals, y_vals, color="red", linewidth=2, label="Particle Trajectory"
)
